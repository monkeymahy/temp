...
class SFDataset(Dataset):  # 显式继承Dataset，增强兼容性与可读性
    ...
    #     if not hasattr(self, "_label_names"):  # 若未缓存类别名
    ...
    #             "slot groove",
    #         ]
    #     return self._label_names  # 返回缓存的类别名称列表

    def label_mapping(self) -> dict[int, int]:
        """返回原始标签到训练标签的映射。"""  # 方法说明
        # mapping = {
        #     0: 2,  # slot groove -> slot groove
        #     1: 0,  # pocket -> other
        #     2: 0,  # planar -> other
        #     3: 1,  # hole -> hole
        ...
        #     5: 0,  # other -> other
        # }
        if not hasattr(self, "lut"):  # 若未缓存映射表
            self.lut = np.asarray([2, 0, 0, 1, 0, 0], dtype=np.int32)  # 构建映射表

        return self.lut  # 返回标签映射表
    ...
    def __init__(  # 初始化数据集
        ...
        # 扫描aag目录下所有json（注意：后续会按split再过滤）
    ...
    def _load_one_graph_with_label(self, fn: str, data: dict) -> dict:
        ...
        # label文件与aag文件分离存放：labels/<fn>.json
        label_path = self.root_dir.joinpath("labels").joinpath(f"{fn}.json")  # 构造标签文件路径
        with open(label_path, "r", encoding="utf-8") as f:  # 显式指定编码，避免跨环境问题
            labels_data = json.load(f)  # 读取标签列表（通常为每个节点一个 int）

        labels_np = np.asarray(labels_data, dtype=np.int32)  # 转为int32的numpy数组
        labels_np = self.label_mapping()[labels_np]  # 应用标签映射（numpy索引更高效）
