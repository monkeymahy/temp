...
class SFDataset(Dataset):  # 显式继承Dataset，增强兼容性与可读性
    ...
    def __init__(  # 初始化数据集
        ...
        label_root = self.root_dir.joinpath(self.label_dir)  # 标签根目录
        if not label_root.exists():  # 训练前提前检查，避免运行中途报错
            raise FileNotFoundError(
                f"标签目录不存在: {label_root}。"
                f"请先运行离线映射脚本生成映射标签，或将 data.label_dir 配置为正确目录。"
            )
    ...
    def _load_one_graph_with_label(self, fn: str, data: dict) -> dict:
        ...
        label_path = self.root_dir.joinpath(self.label_dir).joinpath(f"{fn}.json")  # 构造标签文件路径
