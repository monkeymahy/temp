...
- 面标签和实例编辑都先保存在内存状态中，不会实时落盘。
- 实例编辑会进入撤销/重做栈，可用工具中的撤销/重做快捷键恢复。
- 本次会话修改会在手动点击 `保存GT修改`、切换 STEP、关闭窗口、清空模型或切换 GT 显示状态时聚合成一个版本记录。
...
- 点击版本列表即可预览历史 face 标签和 instance 标签。
- `回滚为新版本` 会以当前最新版本为基线，将所选历史版本保存成一个新的版本记录。
